...
export default function ApiAccessPage() {
  ...
  const [modal, setModal] = useState<{ show: boolean; type: "success" | "error"; email: string; message: string }>({
    show: false, type: "success", email: "", message: ""
  });
      ...
      {Array.isArray(example.code) ? (
        example.code.map((code, index) => (
          <div key={index}>
            ...
            {example.notes?.[index] && <p><i>Note: {example.notes[index]}</i></p>}
          </div>
        ))
      ) : (
    ...
    <Container className="py-4 tab-content mb-0">
      <div className="not-home">
          ...
          <Col md={8}>
            <Form id="apiaccessForm" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <div className="mb-3">
                ...
                <Button type="reset" variant="outline-danger" className="me-1" disabled={loading || isSubmitting} onClick={() => reset()}>
                  Reset
                </Button>
              </div>
            </Form>
        ...
        <div>
          ...
          <p>
            <strong>Note</strong>: <a href="/help#LDassoc">LDassoc</a> is not currently accessible via programmatic access.
          </p>
        </div>
