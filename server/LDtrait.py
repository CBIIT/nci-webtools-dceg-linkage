...
from LDcommon import get_secure_path, sanitize_shell_arg, normalize_chromosome
...
def calculate_trait(snplst, pop, request, web, r2_d, genome_build, r2_d_threshold=0.1, window=500000,ifContinue=True):
    ...
    out_json = open(get_secure_path(tmp_dir, "trait" + str(request) + ".json"), "w")
    output = {}
    ...
    for subprocess_id in range(num_subprocesses):
        with open(get_secure_path(tmp_dir, 'trait_ld_' + str(subprocess_id) + '_' + str(request) + '.txt'), 'w') as snpsPairFile:
            for snps_pair in splitLDPairsUnique[subprocess_id]:
                snpsPairFile.write("\t".join(snps_pair) + "\n")

    ld_subprocess_commands = []
    for subprocess_id in range(num_subprocesses):
        ld_subprocess_commands.append(["python3", "LDtrait_ld_sub.py", sanitize_shell_arg(request), str(subprocess_id), sanitize_shell_arg(genome_build)])
    ...
    for path in Path(tmp_dir).glob("trait_ld_*_" + str(request) + ".txt"):
    ...
    for snp_coord in snp_coords:	
        ...
        if (len(matched_snps) > 0):
            ...
            #output["warning"] = snp_coord[0]+" No entries in the GWAS Catalog are identified using the LDtrait search criteria."+ str(output["warning"] if "warning" in output else "")
