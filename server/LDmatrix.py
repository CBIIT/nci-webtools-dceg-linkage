...
def calculate_matrix(
                ...
                if (snp_i[0][0:2] == "rs" or snp_i[0][0:3] == "chr") and snp_i[0][-1].isdigit():
                    ...
                    if snp_coord != None and snp_coord[genome_build_vars[genome_build]["position"]] != "NA":
                        ...
                        if snp_coord["chromosome"] == "Y" and (
                            genome_build == "grch38" or genome_build == "grch38_high_coverage"
                        ):
    ...
    if warn != []:
        if "warning" in output:
            output["warning"] = (
                output["warning"]
                + ". The following RS number(s) or coordinate(s) inputs have warnings: "
                + ", ".join(warn)
            )
        else:
    ...
    if distance_max > 1000000:
        if "warning" in output:
            output["warning"] = (
                output["warning"]
                + ". Switch rate errors become more common as distance between query variants increases (Query range = "
                + str(distance_max)
                + " bp)"
            )
        else:
    ...
    vcf, head = get_1000g_data(
        snp_pos, snp_coords, genome_build, data_dir + genotypes_dir + genome_build_vars[genome_build]["1000G_dir"]
    )
        ...
        for geno in geno_list:
            ...
            if geno[1] != snp_key:
                mismatch_msg = (
                    "Genomic position ("
                    + geno[1]
                    + ") in 1000G data does not match dbSNP"
                    + dbsnp_version
                    + " ("
            ...
            if "," not in geno[3] and "," not in geno[4]:
                ...
                if len(snp_dict) > 1:
                    output["warning"] = (
                        str(output["warning"] if "warning" in output else "") + rs_input + " variant is not a biallelic"
                    )
                else:
                    output["error"] = (
                        str(output["error"] if "error" in output else "") + rs_input + " variant is not a biallelic"
                    )
                    json_output = json.dumps(output, sort_keys=True, indent=2)
                    print(json_output, file=out_json)
                    out_json.close()
    ...
    if web:
        ...
        if r2_d not in ["r2", "d"]:
            if "warning" in output:
                ...
                output["warning"] = (
                    r2_d + " is not an acceptable value for r2_d (r2 or d required). r2 is used by default"
                )
            r2_d = "r2"
        ...
        if len(snps) < threshold:
            matrix_plot = figure(
                ...
                tools="hover,undo,redo,reset,pan,box_zoom,save",
                title=" ",
                width=800,
                height=700,
            )
        ...
        matrix_rect_renderer = matrix_plot.rect(
            x="xname_pos",
            y="yname",
            width=0.95 * spacing,
            height=0.95,
        ...
        # Connector Plot with dynamic height calculation
        
        # Calculate required height based on maximum SNP ID length
        # Approximating 8pt font width: ~0.6px per character when rotated 90 degrees
        max_snp_id_length = max(len(snp_id) for snp_id in snp_id_plot) if snp_id_plot else 10
        # Base height + additional height for text (convert character length to pixels)
        # Add margin for connector lines and some padding
        base_connector_height = 30 if len(snps) >= threshold else 45
        text_height_needed = max_snp_id_length * 6  # ~6 pixels per character for 8pt font
        dynamic_connector_height = base_connector_height + text_height_needed
        
        # Adjust y-range to accommodate the text
        yr2_adjusted = Range1d(start=0, end=3.8 + (text_height_needed / 20))  # Scale factor for y-range
        yr3_adjusted = Range1d(start=0, end=1 + (text_height_needed / 40))    # Smaller adjustment for larger threshold
        
        if len(snps) < threshold:
            connector = figure(
                outline_line_color="white",
                x_range=xr,
                y_range=yr2_adjusted,
                border_fill_color="white",
                title="",
                min_border_left=100,
                min_border_right=5,
                ...
                min_border_bottom=0,
                width=800,
                height=dynamic_connector_height,
                tools="xpan,tap",
            )
        ...
        connector.xaxis.major_label_text_color = None
        ...
        rug.ygrid.grid_line_color = None
        ...
        if collapseTranscript == "false":
            ...
            genes_json = getRefGene(
                ...
                genes_file,
                snp_coords[1][1],
                int((x[0] - buffer) * 1000000),
                int((x[-1] + buffer) * 1000000),
                genome_build,
                False,
            )
            ...
            gene_plot = figure(
                ...
                width=800,
                height=plot_h_pix,
                tools="hover,xpan,box_zoom,wheel_zoom,tap,undo,redo,reset,save",
            )

            # if len(genes_raw) <= max_genes:
            gene_plot.segment(
                genes_plot_start, genes_plot_yn, genes_plot_end, genes_plot_yn, color="black", alpha=1, line_width=2
            )
            gene_plot.rect(
                x="exons_plot_x",
                y="exons_plot_yn",
            ...
            gene_plot.xaxis.axis_label = (
                "Chromosome " + snp_coords[1][1] + " Coordinate (Mb)(" + genome_build_vars[genome_build]["title"] + ")"
            )
        ...
        # Gene Plot (Collapsed)
            ...
            gene_c_plot = figure(
                ...
                width=900,
                height=plot_c_h_pix,
                tools="hover,xpan,box_zoom,wheel_zoom,tap,undo,redo,reset",
            )
