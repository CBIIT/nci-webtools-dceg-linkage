...
if not logger.handlers:
    ...
    formatter = logging.Formatter("[ldassoc] [%(asctime)s] [%(levelname)s] - %(message)s")
...
def calculate_assoc(file, region, pop, request, genome_build, web, myargs):
    ...
    logger.info(f'ldassoc params {{"file": "{file}", "region": "{region}", "pop": "{pop}", "request": "{request}", "genome_build": "{genome_build}", "web": {web}}}')
    logger.debug("Starting calculate_assoc request")
    ...
    logger.debug(f"Using window size: {window}")
    ...
    if region=="variant":
        ...
        logger.debug(f"Analysis window for variant: chr{chromosome}:{coord1}-{coord2}")
        ...
        # Connect to Mongo snp database
        logger.debug("Connecting to MongoDB for gene lookup")
        db = connectMongoDBReadOnly(web)
        ...
        logger.debug("Processing region mode")
    ...
    logger.debug(f"Executed calculate_assoc ({execution_time}s)")
    return(out_script,out_div)

def main():
    try:
        logger.debug("Starting LDassoc main function")
        ...
        logger.info(f'ldassoc main params {{"file": "{args.file}", "region": "{region}", "pop": "{args.pop}", "request": "{args.request}", "genome_build": "{genome_build}", "web": {web}}}')
        ...
        logger.debug("LDassoc main function completed successfully")
