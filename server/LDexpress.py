    ...
    for snp_coord in snp_coords:
        ...
        if (geno is not None):
            ...
            for subprocess_id in range(num_subprocesses):
                ld_subprocess_commands.append(["python3", "LDexpress_ld_sub.py", str(web), str(snp_coord[0]), str(snp_coord[1]), str(windowChunkRanges[subprocess_id][0]), str(windowChunkRanges[subprocess_id][1]), str(request), str(subprocess_id), str(r2_d), str(r2_d_threshold), str(genome_build)])
            ...
            for subprocess_id in range(num_subprocesses):
                tissues_subprocess_commands.append(["python3", "LDexpress_tissues_sub.py", str(web), str(request), str(subprocess_id), str(p_threshold), str(tissues), str(genome_build)])
