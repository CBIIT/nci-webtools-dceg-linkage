    ...
    for snp_coord in snp_coords:
        ...
        if (geno is not None):
            ...
            for subprocess_id in range(num_subprocesses):
                ld_subprocess_commands.append(["python3", "LDexpress_ld_sub.py", str(web), sanitize_shell_arg(snp_coord[0]), sanitize_shell_arg(snp_coord[1]), str(windowChunkRanges[subprocess_id][0]), str(windowChunkRanges[subprocess_id][1]), sanitize_shell_arg(request), str(subprocess_id), sanitize_shell_arg(r2_d), sanitize_shell_arg(r2_d_threshold), sanitize_shell_arg(genome_build)])
            ld_subprocesses = [subprocess.Popen(command, stdout=subprocess.PIPE) for command in ld_subprocess_commands]
            ...
            for subprocess_id in range(num_subprocesses):
                with open(get_secure_path(tmp_dir, 'express_ld_' + str(subprocess_id) + '_' + str(request) + '.txt'), 'w') as snpsLDFile:
                    for snp_ld_data in windowLDSubsetsChunks[subprocess_id]:
                        snpsLDFile.write("\t".join(snp_ld_data) + "\n")
            tissues_subprocess_commands = []
            for subprocess_id in range(num_subprocesses):
                tissues_subprocess_commands.append(["python3", "LDexpress_tissues_sub.py", str(web), sanitize_shell_arg(request), str(subprocess_id), sanitize_shell_arg(p_threshold), sanitize_shell_arg(tissues), sanitize_shell_arg(genome_build)])
            tissues_subprocesses = [subprocess.Popen(command, stdout=subprocess.PIPE) for command in tissues_subprocess_commands]
            # getTissuesArgs = []   
            # for subprocess_id in range(num_subprocesses): 
        ...
        for path in Path(tmp_dir).glob("express_ld_*_" + str(request) + ".txt"):
            path.unlink(missing_ok=True)
