...
logger = logging.getLogger("ldproxy")
if not logger.handlers:
    handler = logging.StreamHandler()
    formatter = logging.Formatter("[ldproxy] [%(asctime)s] [%(levelname)s] - %(message)s")
...
def calculate_proxy(
    snp, pop, request, web, genome_build, r2_d="r2", window=500000, collapseTranscript=True, annotate="forge"
):

    # Log function entry and parameters
    logger.info(
        f'ldproxy params {{"snp": "{snp}", "pop": "{pop}", "request": "{request}", "web": {web}, "genome_build": "{genome_build}", "r2_d": "{r2_d}", "window": {window}, "collapseTranscript": {collapseTranscript}, "annotate": "{annotate}"}}'
    )
    ...
    if window < 0 or window > 1000000:
        error_msg = "Window value must be a number between 0 and 1,000,000."
        logger.error(f"Invalid window parameter: {window}. {error_msg}")
    ...
    if snp_coord == None or snp_coord[genome_build_vars[genome_build]["position"]] == "NA":
        ...
        logger.error(f"SNP not found in database: {snp}")
        output["error"] = error_msg
        json_output = json.dumps(output, sort_keys=True, indent=2)
        print(json_output, file=out_json)
        out_json.close()
    ...
    if snp_coord["chromosome"] == "Y" and (genome_build == "grch38" or genome_build == "grch38_high_coverage"):
        error_msg = (
            "Input variants on chromosome Y are unavailable for GRCh38, only available for GRCh37 ("
            + "rs"
            + snp_coord["id"]
            + " = chr"
        ...
        logger.error(f"Chromosome Y variant not supported for GRCh38: {snp}")
        output["error"] = error_msg
        json_output = json.dumps(output, sort_keys=True, indent=2)
        print(json_output, file=out_json)
        out_json.close()
    ...
    logger.debug(f"Selecting populations: {pop}")
    pop_ids = get_population(pop, request, output)
    if isinstance(pop_ids, str):
        logger.error("Failed to get population IDs")
    ...
    logger.debug(
        f"Processing query variant: chr{snp_coord['chromosome']}:{snp_coord[genome_build_vars[genome_build]['position']]}"
    )
    ...
    for msg in warningmsg:
        if msg[1] == "NA":
            if "biallelic" in msg[2] or "monoallelic" in msg[2]:
                logger.error(f"Query variant processing failed: {msg[2]}")
    ...
    logger.debug(f"Analysis window: chr{snp_coord['chromosome']}:{coord1}-{coord2}")

    # Calculate proxy LD statistics in parallel
    windowChunkRanges = chunkWindow(
        int(snp_coord[genome_build_vars[genome_build]["position"]]), window, num_subprocesses
    )
    logger.debug(f"Created {num_subprocesses} subprocess chunks for parallel processing")
    ...
    for i in range(len(out_raw)):
        for j in range(len(out_raw[i])):
            ...
            out_prox.append(col)
    logger.debug(f"Aggregated {len(out_prox)} proxy variants")
    # Sort output
    if r2_d not in ["r2", "d"]:
        warning_msg = r2_d + " is not an acceptable value for r2_d (r2 or d required). r2 is used by default"
        logger.warning(f"Invalid r2_d parameter '{r2_d}', using default 'r2'")
    ...
    temp = [
        ...
        query_snp["MAF"],
        str(query_snp["Dist"]),
        str(query_snp["Dprime"]),
        str(query_snp["R2"]),
        query_snp["Corr_Alleles"],
        query_snp["ForgeDB"] if len(query_snp["ForgeDB"]) > 0 else "NA",
        query_snp["RegulomeDB"],
        query_snp["Function"],
    ]
    ...
    for i in range(1, len(out_ld_sort)):
        if float(out_ld_sort[i][8]) > 0.01 and out_ld_sort[i][3] != snp:
            ...
            temp = [
                ...
                proxy_info["MAF"],
                str(proxy_info["Dist"]),
                str(proxy_info["Dprime"]),
                str(proxy_info["R2"]),
                proxy_info["Corr_Alleles"],
                proxy_info["ForgeDB"] if len(proxy_info["ForgeDB"]) > 0 else "NA",
                proxy_info["RegulomeDB"],
                proxy_info["Function"],
            ]
            print("\t".join(temp), file=outfile)
    ...
    if r2_d == "r2":
        print(
            'track type=bedGraph name="R2 Plot" description="Plot of R2 values" color=50,50,50 visibility=full alwaysZero=on graphType=bar maxHeightPixels=60',
            file=track,
        )
    else:
        print(
            'track type=bedGraph name="D Prime Plot" description="Plot of D prime values" color=50,50,50 visibility=full alwaysZero=on graphType=bar maxHeightPixels=60',
            file=track,
        )

    print("\t".join([str(ucsc_track["query_snp"][i]) for i in [0, 1, 1, 3]]), file=track)
    ...
    print('track type=bed name="' + snp + '" description="Query Variant: ' + snp + '" color=108,108,255', file=track)
    print("\t".join([ucsc_track["query_snp"][i] for i in [0, 1, 1, 2]]), file=track)
    print("", file=track)

    if len(ucsc_track["0.8-1.0"]) > 0:
        if r2_d == "r2":
            print(
                'track type=bed name="0.8<R2<=1.0" description="Proxy Variants with 0.8<R2<=1.0" color=198,129,0',
                file=track,
            )
        else:
            print(
                'track type=bed name="0.8<D\'<=1.0" description="Proxy Variants with 0.8<D\'<=1.0" color=198,129,0',
                file=track,
            )
        for var in ucsc_track["0.8-1.0"]:
    ...
    if len(ucsc_track["0.6-0.8"]) > 0:
        if r2_d == "r2":
            print(
                'track type=bed name="0.6<R2<=0.8" description="Proxy Variants with 0.6<R2<=0.8" color=198,129,0',
                file=track,
            )
        else:
            print(
                'track type=bed name="0.6<D\'<=0.8" description="Proxy Variants with 0.6<D\'<=0.8" color=198,129,0',
                file=track,
            )
        for var in ucsc_track["0.6-0.8"]:
            print("\t".join([var[i] for i in [0, 1, 1, 2]]), file=track)
    ...
    if len(ucsc_track["0.4-0.6"]) > 0:
        if r2_d == "r2":
            print(
                'track type=bed name="0.4<R2<=0.6" description="Proxy Variants with 0.4<R2<=0.6" color=198,129,0',
                file=track,
            )
        else:
            print(
                'track type=bed name="0.4<D\'<=0.6" description="Proxy Variants with 0.4<D\'<=0.6" color=198,129,0',
                file=track,
            )
        for var in ucsc_track["0.4-0.6"]:
            print("\t".join([var[i] for i in [0, 1, 1, 2]]), file=track)
    ...
    if len(ucsc_track["0.2-0.4"]) > 0:
        if r2_d == "r2":
            print(
                'track type=bed name="0.2<R2<=0.4" description="Proxy Variants with 0.2<R2<=0.4" color=198,129,0',
                file=track,
            )
        else:
            print(
                'track type=bed name="0.2<D\'<=0.4" description="Proxy Variants with 0.2<D\'<=0.4" color=198,129,0',
                file=track,
            )
        for var in ucsc_track["0.2-0.4"]:
            print("\t".join([var[i] for i in [0, 1, 1, 2]]), file=track)
    ...
    if len(ucsc_track["0.0-0.2"]) > 0:
        if r2_d == "r2":
            print(
                'track type=bed name="0.0<R2<=0.2" description="Proxy Variants with 0.0<R2<=0.2" color=198,129,0',
                file=track,
            )
        else:
            print(
                'track type=bed name="0.0<D\'<=0.2" description="Proxy Variants with 0.0<D\'<=0.2" color=198,129,0',
                file=track,
            )
        for var in ucsc_track["0.0-0.2"]:
            print("\t".join([var[i] for i in [0, 1, 1, 2]]), file=track)
    ...
    if web:
        ...
        logger.debug("Generating visualization plots for web interface")

        # out_grid = ldproxy_figue(out_ld_sort, r2_d,coord1,coord2,snp,pop,request,db,snp_coord,genome_build,collapseTranscript)
        out_grid, proxy_plot, gene_plot, rug, plot_h_pix = ldproxy_figure(
            out_ld_sort,
            r2_d,
            coord1,
            coord2,
        ...
        def send_async():
            try:
                ...
                with httpx.Client(timeout=None) as client:
                    client.post("http://localhost:5000/ldproxy_svg", json=payload, headers=headers, timeout=None, follow_redirects=True)
                logger.debug("High quality image export request completed successfully")
        ...
        logger.debug(f"Executed calculate_proxy ({duration:.2f}s)")

    # Return plot output
    execution_time = round(time.time() - start_time, 2)
    logger.debug(f"Executed calculate_proxy ({execution_time}s)")
    return (out_script, out_div)


def main():
    try:
        logger.debug("Starting LDproxy main function")
        tmp_dir = "./tmp/"
        ...
        if len(sys.argv) == 5:
            ...
            error_msg = "Correct useage is: LDproxy.py snp populations request (optional: r2_d)"
            logger.error(f"Invalid command line arguments: {sys.argv}")
            print(error_msg)
            sys.exit()

        logger.info(
            f'ldproxy main params {{"snp": "{snp}", "pop": "{pop}", "request": "{request}", "web": {web}, "r2_d": "{r2_d}"}}'
        )
        ...
        try:
            ...
            print("\t".join(head))
            temp = [
                json_dict["query_snp"]["RS"],
                json_dict["query_snp"]["Coord"],
                json_dict["query_snp"]["Alleles"],
                json_dict["query_snp"]["MAF"],
                str(json_dict["query_snp"]["Dist"]),
                str(json_dict["query_snp"]["Dprime"]),
                str(json_dict["query_snp"]["R2"]),
                json_dict["query_snp"]["Corr_Alleles"],
                json_dict["query_snp"]["RegulomeDB"],
                json_dict["query_snp"]["Function"],
            ]
            print("\t".join(temp))
            for k in sorted(json_dict["proxy_snps"].keys())[0:10]:
                temp = [
                    json_dict["proxy_snps"][k]["RS"],
                    json_dict["proxy_snps"][k]["Coord"],
                    json_dict["proxy_snps"][k]["Alleles"],
                    json_dict["proxy_snps"][k]["MAF"],
                    str(json_dict["proxy_snps"][k]["Dist"]),
                    str(json_dict["proxy_snps"][k]["Dprime"]),
                    str(json_dict["proxy_snps"][k]["R2"]),
                    json_dict["proxy_snps"][k]["Corr_Alleles"],
                    json_dict["proxy_snps"][k]["RegulomeDB"],
                    json_dict["proxy_snps"][k]["Function"],
                ]
                print("\t".join(temp))
            print("")
        ...
        logger.debug("LDproxy main function completed successfully")
